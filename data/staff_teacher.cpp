...
int main()
{
	const char output_file[] = "sql\\staff_teacher.sql";
	const char input_file[] = "text\\teachernames.txt";
	ofstream outfile;
	ifstream infile;
	...
	for (int i = 1; i <= rows; i++) {
		...
		sprintf(buffer, "insert into %s (%s, %s, %s) values ('%s', '%s', %d);\n", 
                table, param[0], param[1], param[2], line, position, i + (rows/10));
		outfile << buffer;
